...
func NewRouter(routers ...Router) *mux.Router {
	...
	for _, api := range routers {
		for name, route := range api.Routes() {
			...
			if route.Method == "POST" && name != "CreateUser" && strings.HasSuffix(route.Pattern, "v2") {
				route.Pattern += "/"
			}
			if route.Method == "PUT" && strings.HasSuffix(route.Pattern, "/") ||
				route.Method == "DELETE" && strings.HasSuffix(route.Pattern, "/") {
				route.Pattern = route.Pattern[:len(route.Pattern)-len("/")]
			}
