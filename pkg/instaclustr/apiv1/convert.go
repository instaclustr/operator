...
func PgToInstAPI(pgClusterSpec *v1alpha1.PgSpec) *PgCluster {
	...
	if isSingleDC {
		pgRackAllocation := &RackAllocationAPIv1{
...
func pgProviderToInstAPI(dataCentre *v1alpha1.PgDataCentre) *ClusterProviderAPIv1 {
	return &ClusterProviderAPIv1{
...
func dataCentresFromInstAPIv1(instaDataCentres []*DataCentreStatusAPIv1) []*v1alpha1.DataCentreStatus {
...
func nodesFromInstAPIv1(instaNodes []*NodeStatusAPIv1) []*v1alpha1.Node {
...
func firewallRulesToInstAPI(firewallRules []*v1alpha1.FirewallRule) []*FirewallRuleAPIv1 {
	if len(firewallRules) < 1 {
		return nil
	}

	var instFirewallRules []*FirewallRuleAPIv1

	for _, firewallRule := range firewallRules {
		var instRules []*RuleTypeAPIv1
		for _, rule := range firewallRule.Rules {
			instRule := &RuleTypeAPIv1{
				Type: rule.Type,
			}
			instRules = append(instRules, instRule)
		}

		instFirewallRule := &FirewallRuleAPIv1{
...
func pgTwoFactorDeleteToInstAPI(twoFactorDelete []*v1alpha1.TwoFactorDelete) *TwoFactorDeleteAPIv1 {
	if len(twoFactorDelete) < 1 {
		return nil
	}

	return &TwoFactorDeleteAPIv1{
